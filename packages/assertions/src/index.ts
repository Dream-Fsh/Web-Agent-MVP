  ...
  for (const assertion of assertions) {
    try {
      if (assertion.type === "assertUrl") {
        ...
        if (assertion.type === "assertCount") {
          const target = await resolveCollectionTarget(page, assertion.target);
          if (typeof assertion.expected !== "number" || target.locators.length !== assertion.expected) throw new Error("Target count did not match");
        } else {
          const target = await resolveSingleTarget(page, assertion.target);
