...
export async function extractText(page: Page, target: Target): Promise<string> { const resolved = await resolveTarget(page, target); return (await resolved.locator.textContent()) ?? ""; }
export async function extractAttribute(page: Page, target: Target, attribute: string): Promise<string | null> { const resolved = await resolveTarget(page, target); return resolved.locator.getAttribute(attribute); }
export async function extractList(page: Page, target: Target): Promise<string[]> { const resolved = await resolveTarget(page, target); return resolved.locator.allTextContents(); }
export async function extractCount(page: Page, target: Target): Promise<number> { const resolved = await resolveTarget(page, target); return resolved.locator.count(); }
...
export async function extractPaginatedTable(page: Page, tableTarget: Target, nextTarget: Target, maxPages = 100): Promise<ExtractedTable> {
  ...
  for (let pageIndex = 0; pageIndex < maxPages; pageIndex += 1) {
    ...
    let next;
    try { next = await resolveTarget(page, nextTarget); } catch { break; }
